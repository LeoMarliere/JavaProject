...
public class Bibliotheque implements IBibliotheque {
	 ...
	 *             if no book in the library has the given id
	 * @throws UnavailableBookException
	 *             if all books in the library with the given id have been borrowed
	 */
	public void borrowBook(String id, String username) throws LibraryException {
		try {
							...
							if (!user.get(j).getBook().contains(book.get(i))) {
								if (book.get(i).getNbExemplaire() <= 0) {
									throw new LibraryException(
											"Le livre " + book.get(i).getTitre() + " n'est plus disponible");
								}
			...
			// throw new LibraryException("le livre que vous chercher n'est pas dans notre
			// bibliothÃ¨que");
			System.out.println("Erreur Appel meÌ�thode boorowBook() : " + e.getMessage());
		}
	}
	 ...
	 * @throws BookNotFoundException
	 *             if no book in the library has the given id
	 * @throws AllBooksAlreadyReturnedException
	 *             if all books with the given id are already returned
	 */
	public void returnBook(String id, String username) throws LibraryException {
		try {
					...
					while (it2.hasNext()) {
						if (user.get(j).getUsername() == username) {
							if (user.get(j).getBook().contains(book.get(i))) {
								...
								throw new LibraryException(
										username + " a deja rendu le livre : " + book.get(i).getTitre());
							}
						}
						j++;
						it2.next();
			...
			System.out.println("Erreur Appel meÌ�thode returnBook() : " + e.getMessage());
		}
